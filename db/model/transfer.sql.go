...
RETURNING id, from_account_id, to_account_id, amount, coin, created_at
`
...
SELECT id, from_account_id, to_account_id, amount, coin, created_at FROM transfers
WHERE from_account_id = $1 
LIMIT 100
`

func (q *Queries) GetTransfers(ctx context.Context, fromAccountID int64) ([]Transfer, error) {
	rows, err := q.query(ctx, q.getTransfersStmt, getTransfers, fromAccountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
